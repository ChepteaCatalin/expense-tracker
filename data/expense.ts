...
export const createExpense = authGuard(
  session =>
    async (expense: TransactionFormValues): Promise<Transaction> => {
      ...
      updateTag(`expenses/categories/user/${session.user.id}`);
      updateTag(
...
export const deleteExpense = authGuard(
  session =>
    async (expenseId: number): Promise<{ id: number; categoryId: number }> => {
      ...
      updateTag(`expenses/categories/user/${session.user.id}`);
      updateTag(
        `expenses/category/${deletedExpense.category_id}/user/${session.user.id}`,
      );
      updateTag(`expenses/id/${expenseId}/user/${session.user.id}`);
...
export const getExpenseCategories = authGuard(
  session =>
    async ({
      ...
      'use cache';
      cacheLife('minutes');
      cacheTag(`expenses/categories/user/${session.user.id}`);
...
export const getExpenseById = authGuard(
  session =>
    async (expenseId: number): Promise<Transaction | undefined> => {
      'use cache';
      cacheLife('minutes');
      cacheTag(`expenses/id/${expenseId}/user/${session.user.id}`);
...
export const getExpenseCategoryTotal = authGuard(
  session =>
    async ({
      ...
      'use cache';
      cacheLife('minutes');
      cacheTag(`expenses/category/${categoryId}/user/${session.user.id}`);
